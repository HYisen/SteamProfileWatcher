...
func ParseCSVLine(line string) (date string, stat GameStat, err error) {
	...
	if err != nil {
		return "", GameStat{}, fmt.Errorf("parse PlayTimeTwoWeeksMinutes: %w", err)
	}
	stat.PlayTimeForeverMinutes, err = strconv.Atoi(record[len(record)-1])
	if err != nil {
		return "", GameStat{}, fmt.Errorf("parse PlayTimeForeverMinutes: %w", err)
	}
	return date, stat, nil
}
